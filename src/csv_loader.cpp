...
{
CSVLoader::CSVLoader(
  const std::string & csv_path, const std::string delimiter,
...
bool CSVLoader::validateData(const Table & table, const std::string & csv_path)
{
  ...
  auto max_cols = getMaxCols();
  if (table[0].size() < max_cols) {
    std::cerr << "Cannot read " << csv_path.c_str() << " CSV file should have at least "
              << max_cols << " columns. It contains " << table[0].size() << " columns."
              << std::endl;
    return false;
  }
  // validate map size
...
size_t CSVLoader::getMaxCols()
{
  std::vector<size_t> idxs = {col_x_, col_y_, col_yaw_, col_vel_};
  return *std::max_element(idxs.begin(), idxs.end()) + 1;
}

Points CSVLoader::getPoints(const Table & table)
{
  Points points = {};
  for (size_t i = idx_begin_; i < table.size(); i++) {
    ...
    point.push_back(std::stod(table[i][col_yaw_]) + yaw_offset_);
